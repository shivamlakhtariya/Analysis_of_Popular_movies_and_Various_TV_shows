...
for page in range(1,55):  #take 50 pages from json
    response = requests.get("https://api.themoviedb.org/3/tv/popular?api_key=e3ddc5c5e55adb9e4ce344df5400aeea&language=en-US&page="+str(page))
...
for show in range(0,len(tv_shows_list)):   #take 1000 tv shows list from api
    response = requests.get("https://api.themoviedb.org/3/tv/"+str(tv_shows_list[show])+"/reviews?api_key=e3ddc5c5e55adb9e4ce344df5400aeea")
    if response.status_code == 200:
       json_data[count] = response.json()
       count = count+1

###########################################MongoDB Connecivity#########################################################################
dbconn = pm("mongodb://dapgrpl:dapgrpl@18.203.68.115/")
tmdb = dbconn["TMDB"]
...
sqlio.to_sql(Dataframe_Reviews, 'tv_review', 'postgresql+psycopg2://dapgrpl:dapgrpl@18.203.68.115/tmdb', if_exists = 'replace')
